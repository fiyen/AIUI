...
interface RealTimeVADCallbacks {
  ...
  /**
   * Callback to run during all the function life.
   * Takes as arg a Float32Array of audio samples between -1 and 1, sample rate 16000.
   * This function will get audio samples of a short interval, like 40ms, and is called
   * every 40ms. 
   */
  onSpeechDoing?: (audioList: Float32Array[]) => any
}
...
export const defaultRealTimeVADOptions: RealTimeVADOptions = {
  ...
  onSpeechDoing: () => {
    log.debug("Detected speech doing")
  },
...
export class MicVAD {
  ...
  private startSpeechDoing = () => {
    this.audioNodeVAD.startSpeechDoing();
  };

  private stopSpeechDoing = () => {
    this.audioNodeVAD.stopSpeechDoing();
  };

  pause = () => {
    this.audioNodeVAD.pause()
    this.listening = false
    this.stopSpeechDoing()
  }
...
export class AudioNodeVAD {
  ...
  private speechDoingInterval?: number;
  private ongoingAudioData: Float32Array[] = [];

  public startSpeechDoing = () => {
    if (this.speechDoingInterval !== undefined) return; // prevent starting twice
    this.speechDoingInterval = window.setInterval(() => {
      if (this.options.onSpeechDoing && this.ongoingAudioData.length > 0) {
        // console.log(this.ongoingAudioData)
        this.options.onSpeechDoing(this.ongoingAudioData); // send earliest collected audio data
        this.ongoingAudioData = [];
      }
    }, 46);  
  };

  public stopSpeechDoing = () => {
    if (this.speechDoingInterval !== undefined) {
      clearInterval(this.speechDoingInterval); // clear the timer
      this.speechDoingInterval = undefined;
    }
  };
  ...
  processFrame = async (frame: Float32Array) => {
    ...
    this.handleFrameProcessorEvent(ev)
    // Store ongoing frames for onSpeechDoing.
    this.ongoingAudioData.push(frame)
  }
  ...
  handleFrameProcessorEvent = (
    ...
    switch (ev.msg) {
      case Message.SpeechStart:
        ...
        this.ongoingAudioData = []; // reset collected audio data
        this.startSpeechDoing(); // start onSpeechDoing timer
        break

      case Message.VADMisfire:
        this.options.onVADMisfire()
        this.stopSpeechDoing(); // stop onSpeechDoing timer
        this.ongoingAudioData = []; // reset collected audio data
        break
